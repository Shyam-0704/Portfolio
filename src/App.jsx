  ...
  const projects = [
    {
      ...
      description:
        "Built an automated system to detect and block malicious IPs using threat intelligence APIs, dynamic allowlist and banlist logic, and real-time alerting."
    },
    {
      title: "API Request Authentication & Anti-Tampering",
      description:
        "Designed a request authentication flow using HMAC signatures, endpoint and method binding, and timestamp validation to reduce tampering and replay risks."
    }
  ];

  const certifications = ["Certified Ethical Hacker (CEH)", "Python Certification"];
    ...
    <div className="min-h-screen overflow-x-hidden bg-[#050816] text-slate-100">
      <div className="pointer-events-none fixed inset-0">
        <motion.div
          ...
          className="absolute left-10 top-10 h-72 w-72 rounded-full bg-cyan-500/10 blur-3xl"
        />
        <motion.div
          animate={{ x: [0, -30, 0], y: [0, -20, 0] }}
          transition={{ duration: 12, repeat: Infinity, ease: "easeInOut" }}
          className="absolute right-10 top-40 h-80 w-80 rounded-full bg-indigo-500/10 blur-3xl"
        />
        <motion.div
          animate={{ x: [0, 20, 0], y: [0, -35, 0] }}
          transition={{ duration: 14, repeat: Infinity, ease: "easeInOut" }}
          className="absolute bottom-10 left-1/3 h-72 w-72 rounded-full bg-emerald-500/10 blur-3xl"
        />
      </div>
      ...
      <main className="relative mx-auto max-w-6xl px-6 py-10 md:px-8 lg:px-10">
        <motion.section
          ...
          animate="show"
          variants={stagger}
          className="rounded-[28px] border border-white/10 bg-white/5 p-8 shadow-2xl backdrop-blur md:p-12"
        >
          <div className="grid gap-10 lg:grid-cols-[1.25fr_0.75fr] lg:items-center">
            <div>
              <motion.div
                variants={fadeUp}
                className="mb-4 inline-flex items-center rounded-full border border-cyan-400/30 bg-cyan-400/10 px-4 py-1 text-sm text-cyan-300"
              >
              ...
              >
                Junior Security Engineer focused on application security, web and
                API testing, threat modeling, and DevSecOps integration. I work
                across sprint security validation, secure SDLC practices, API
                protection mechanisms, and cloud security monitoring.
              </motion.p>
              ...
              >
                ...
                <span className="rounded-full border border-white/10 bg-black/20 px-4 py-2">
                  AppSec • DevSecOps • Cloud Security
                </span>
              </motion.div>
              ...
              <motion.div variants={fadeUp} className="mt-8 flex flex-wrap gap-4">
                <a
                  ...
                  className="rounded-2xl bg-cyan-500 px-5 py-3 font-medium text-slate-950 transition hover:scale-[1.04]"
                >
                  Contact Me
                </a>
                <a
              ...
              <div className="mt-5 space-y-4 text-sm text-slate-300">
                <div>
                  ...
                  <p className="mt-1 text-white">
                    Threat Modeling, Sprint Release Security, API Security
                  </p>
                </div>
                <div>
                  <p className="text-slate-400">DevSecOps</p>
                  <p className="mt-1 text-white">
                    SonarQube, Dependency Scanning, Secure SDLC Support
                  </p>
                </div>
                <div>
                  <p className="text-slate-400">Cloud Security</p>
                  <p className="mt-1 text-white">
                    Cloudflare Monitoring, AWS Inspector, GuardDuty, Security Hub
                  </p>
                </div>
                <div>
                  <p className="text-slate-400">Methodologies</p>
                  <p className="mt-1 text-white">
                    OWASP Top 10, STRIDE, MITRE ATT&CK, NIST
                  </p>
                </div>
              </div>
            </motion.div>
        ...
        >
          <motion.div
            variants={fadeUp}
            className="rounded-[28px] border border-white/10 bg-white/5 p-7 lg:col-span-2"
          >
            <h2 className="text-2xl font-semibold text-white">About Me</h2>
            <p className="mt-4 leading-7 text-slate-300">
              I primarily work on application security activities within sprint
              cycles, including threat modeling for premium sprint tasks, security
              testing for sprint releases, and collaborating with developers to
              implement secure features. I also contribute to DevSecOps through
              dependency and code scanning, review Cloudflare events and logs,
              support documentation, participate in team security discussions, and
              continuously expand my knowledge in modern security practices.
            </p>
          </motion.div>
        ...
        <motion.section
          ...
          viewport={{ once: true, amount: 0.2 }}
          variants={stagger}
          className="mt-10 rounded-[28px] border border-white/10 bg-white/5 p-7 md:p-8"
        >
          ...
          <motion.div
            variants={fadeUp}
            whileHover={{ y: -4 }}
            className="mt-6 rounded-[28px] border border-cyan-400/20 bg-cyan-400/5 p-6"
          >
            ...
            <motion.ul variants={stagger} className="mt-5 grid gap-3 text-slate-300 md:grid-cols-2">
              {experience.map((item) => (
                <motion.li
                  key={item}
                  variants={fadeUp}
          ...
          <motion.div
            variants={fadeUp}
            className="mt-6 rounded-[28px] border border-white/10 bg-black/20 p-6"
          >
            <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
              <div>
        ...
        <motion.section
          ...
          className="mt-10 rounded-[28px] border border-white/10 bg-gradient-to-r from-cyan-500/10 to-indigo-500/10 p-8 text-center"
        >
          ...
          <p className="mx-auto mt-3 max-w-2xl leading-7 text-slate-300">
            I’m currently exploring opportunities in Application Security, Product
            Security, and related cybersecurity roles where I can contribute to
            secure SDLC, threat modeling, testing, and continuous security
            improvement.
          </p>
